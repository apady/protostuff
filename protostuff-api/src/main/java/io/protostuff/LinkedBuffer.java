...
public final class LinkedBuffer
{
    ...
    /**
     * Writes the contents of the {@link LinkedBuffer} into the {@link ByteBuf}.
     *
     * @return the total content size of the buffer.
     */
    public static int writeTo(final ByteBuf byteBuf, LinkedBuffer node) throws IOException
    {
        int contentSize = 0, len;
        do
        {
            if ((len = node.offset - node.start) > 0)
            {
                byteBuf.writeBytes(node.buffer, node.start, len);
                contentSize += len;
            }
        } while ((node = node.next) != null);
